...
export interface ServerToClientEvents {
    gamestate_updated: (state: GameState) => void;
    users_updated: (users: Omit<User, 'passwordHash' | 'securityAnswerHash'>[]) => void;
    initial_state: (data: { gameState: GameState, users: Omit<User, 'passwordHash' | 'securityAnswerHash'>[] }) => void;
    login_success: (user: Omit<User, 'passwordHash' | 'securityAnswerHash'>) => void;
    login_fail: (message: string) => void;
    register_fail: (message: string) => void;
    security_question: (question: string | null) => void;
    answer_verified: (isCorrect: boolean) => void;
    reset_password_success: (message: string) => void;
    reset_password_fail: (message: string) => void;
    alert: (message: string) => void;
}

export interface ClientToServerEvents {
    get_initial_state: () => void;
    login: (data: { username: string, password: string }) => void;
    register: (data: { username: string, password: string, securityQuestion: string, securityAnswer: string }) => void;
    get_security_question: (username: string) => void;
    verify_security_answer: (data: { username: string, answer: string }) => void;
    reset_password: (data: { username: string, newPassword: string }) => void;
    create_tribe: (tribeData: Omit<Tribe, 'id' | 'turnSubmitted' | 'actions' | 'lastTurnResults' | 'exploredHexes' | 'rationLevel' | 'completedTechs' | 'assets' | 'currentResearch' | 'journeyResponses' | 'diplomacy'>) => void;
    submit_turn: (data: { tribeId: string, plannedActions: GameAction[], journeyResponses: Tribe['journeyResponses'] }) => void;
    process_turn: () => void;
    update_tribe: (updatedTribe: Tribe) => void;
    remove_player: (userId: string) => void;
    start_new_game: () => void;
    load_backup: (backup: FullBackupState) => void;
    update_map: (data: { newMapData: HexData[], newStartingLocations: string[] }) => void;
    request_chief: (data: { tribeId: string, chiefName: string, radixAddressSnippet: string }) => void;
    approve_chief: (requestId: string) => void;
    deny_chief: (requestId: string) => void;
    request_asset: (data: { tribeId: string, assetName: string, radixAddressSnippet: string }) => void;
    approve_asset: (requestId: string) => void;
    deny_asset: (requestId: string) => void;
    add_ai_tribe: () => void;
    propose_alliance: (data: { fromTribeId: string, toTribeId: string }) => void;
    sue_for_peace: (data: { fromTribeId: string, toTribeId: string, reparations: { food: number, scrap: number, weapons: number } }) => void;
    accept_proposal: (proposalId: string) => void;
    reject_proposal: (proposalId: string) => void;
    declare_war: (data: { fromTribeId: string, toTribeId: string }) => void;
}
